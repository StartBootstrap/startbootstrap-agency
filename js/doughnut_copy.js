...
for (var i = 0; i < doughnutArray.length-2; i++) {
...
for (var i = 4; i < doughnutArray.length; i++) {
  doughnutArray[i].lineWidth = 5; //thickness of the line
  doughnutArray[i].fillStyle = '#eaeaea';
  doughnutArray[i].strokeStyle = "#eaeaea";
  doughnutArray[i].beginPath();
  doughnutArray[i].arc(120, 60, 55, 4.72,15, false); //.arc(x, y , radius, startAngle, endAngle, anticlockwise)
  doughnutArray[i].stroke();
}
...
function loadSkills5() {
  ...
  function progressSim() {
    ...
    ctx.fillText(al + '%', cw * .5 +2, ch * .5 + 8, cw);
    ctx.beginPath();
    ctx.arc(0, 60, 55, start, diff / 10 + start, false); //.arc(x, y , radius, startAngle, endAngle, anticlockwise)
    ctx.stroke();
